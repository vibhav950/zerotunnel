...
static const uint8_t AUTHKEY[32] = {0x01};

static const uint8_t STREAMID[8] = {0x02};

static const uint8_t plaintext1[16] = {0x03};
static const uint8_t plaintext2[32] = {0x04};
...
static void initiator_process(int read_fd, int write_fd) {
  ...
  write_len = sizeof(plaintext1) + vcry_get_aead_tag_len(ctx);
  ...
  uint8_t *buf = zt_malloc(sizeof(plaintext2));
  ...
  buf = zt_malloc(sizeof(plaintext1));
  ...
  ASSERT_MEMEQ(buf, plaintext1, sizeof(plaintext1));
...
static void responder_process(int read_fd, int write_fd) {
  ...
  uint8_t *buf = zt_malloc(sizeof(plaintext1));
