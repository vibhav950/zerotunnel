...
static void tty_initfp(void) {
  ...
  if (!ttyfp)
    PRINTFATAL("cannot open '%s' (%s)", tty_get_name(), strerror(errno));
  atexit(cleanup);
}
...
static char *do_get(const char *prompt, int hidden) {
  ...
  if (no_terminal)
    PRINTFATAL("no terminal available -- can't get input");
  ...
  if (hidden) {
    ...
    if (tcgetattr(fileno(ttyfp), &termios_save) != 0)
      PRINTFATAL("tcgetattr() failed (%s)", strerror(errno));
    restore_termios = 1;
    term = termios_save;
    term.c_lflag &= ~(ECHO | ECHOE | ECHOK | ECHONL);
    if (tcsetattr(fileno(ttyfp), TCSANOW, &term) != 0)
      PRINTFATAL("tcsetattr() failed (%s)", strerror(errno));
  }
