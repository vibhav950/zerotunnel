 ...
 * `liboqs_kem_dealloc()` after use to prevent leaking of sensitive data.
 */
 ...
 * @param[in] ct_len Length of the ciphertext
 * @param[out] ss Pointer to shared secret buffer (allocated by the function)
 * @param[out] ss_len Length of the shared secret buffer
 * @return ERR_SUCCESS on success, error code otherwise
 *
 * @note The memory allocated in @p ss must be freed by calling `liboqs_kem_mem_free()`
 * after use.
 */
